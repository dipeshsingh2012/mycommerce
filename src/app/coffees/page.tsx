...
import { fetchProducts, ProductItem } from '@/lib/catalogApi';
  ...
  useEffect(() => {
    async function loadCoffees() {
      try {
        ...
        // Only specialty coffee lots (exclude equipment)
        const coffees = data.filter((p) => p.category !== 'equipment');
        setProducts(coffees);
      } catch (err) {
        console.error('Failed to load coffees from catalog service:', err);
      } finally {
        setLoading(false);
      }
    }
    ...
    <div className="space-y-8 py-6">
      ...
      {loading ? (
        <div className="py-20 text-center space-y-3">
          ...
          <p className="text-xs font-semibold text-slate-500">Loading fresh roastery harvest from catalog service...</p>
        </div>
